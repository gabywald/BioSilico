...
public class Organism extends Agent implements AgentContent {
	...
	public Organism(Chromosome genome[]) {
		super(true,true,true);
		this.genome	= new ChromosomeListe(genome);
		this.init();
	}
	 ...
	 */
	public Organism(ChromosomeListe genome) {
		super(true,true,true);
		this.genome	= genome;
		this.init();
	}
	...
	public Organism(Chromosome genome) {
		super(true,true,true);
		this.genome	= new ChromosomeListe();
		this.genome.addChromosome(genome);
		this.init();
	}
	...
	private void init() {
		...
		this.geneticCodes	= new StringCoupleListe();
		this.currentBrain	= null;
		this.liste			= new AgentListe();
		this.geneticCodes.addStringCouple(new StringCouple("0000000010",
														  	"Gattaca01"));
		this.extendedlineage = new ExtendedLineage();
	}
	
	public Brain getBrain() 			{ return this.currentBrain; }
	public ChromosomeListe getGenome() 	{ return this.genome; }
	...
	public StringListe getSimpleLinage() 
	...
	public StringCoupleListe getGeneticCodes()
	...
	public void execution(WorldCase local) {
		...
		for (int i = 0 ; i < this.genome.length() ; i++) 
			{ this.genome.getChromosome(i).execution(this); }
	...
	private void decisionToGet(int object) 
		{ this.liste.addAgent(this.current.getAgentType(object)); }
	...
	public String toString() {
		...
		/** Current instance. */
		result += "GENETIC CODE\n\tID\t"
					+this.geneticCodes.getStringCouple(0).getValueA()+"\n";
		result += "\tNAME\t"
					+this.geneticCodes.getStringCouple(0).getValueB()+"\n";
		for (int i = 1 ; i < this.geneticCodes.length() ; i++) {
			result += "SUB GENETIC CODE\n\tID\t"
					+this.geneticCodes.getStringCouple(i).getValueA()+"\n";
			result += "\tNAME\t"
					+this.geneticCodes.getStringCouple(i).getValueB()+"\n";
		}
		result += "LINEAGE\n";
		...
		result += "GENOME\n";
		for (int i = 0 ; i < this.genome.length() ; i++) {
			Chromosome chr = this.genome.getChromosome(i);
