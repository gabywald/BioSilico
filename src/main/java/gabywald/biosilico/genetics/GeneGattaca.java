...
public abstract class GeneGattaca extends Gene {
	...
	public static GeneGattaca getInstance(String sequence) {
		...
		for (int i = 0 ; i < 3 ; i++) {
			...
			if (diff > 15) { /** 15 is length of translated header */
				...
				int sex = 0;
				int mut_rate = 0;
				IntegerListe memory = new IntegerListe();
				...
				for (int j = start_index ; j < stops_index ; j++) {
					...
					if (j > 16) {
						if (Character.isDigit(test_char)) 
						{ memory.addInteger(
								new Integer(Integer.parseInt(test_char+""))); } 
						else { return null; }
					}
	...
	private static InitialConcentration getIC(
			...
			int age_min,int age_max,int sex,int mut_rate,IntegerListe memory) {
		int var = memory.getInteger(0).intValue()*100
				+ memory.getInteger(1).intValue()*10
				+ memory.getInteger(2).intValue()*1;
		int val = memory.getInteger(3).intValue()*100
				+ memory.getInteger(4).intValue()*10
				+ memory.getInteger(5).intValue()*1;
	...
	private static BiochemicalReaction getBR(
			boolean mutate,boolean duplicate,boolean delete,boolean activ,
			int age_min,int age_max,int sex,int mut_rate,IntegerListe memory) {
		int Achem = memory.getInteger(0).intValue()*100
					+ memory.getInteger(1).intValue()*10
					+ memory.getInteger(2).intValue()*1;
		int Acoef = memory.getInteger(3).intValue()*100
					+ memory.getInteger(4).intValue()*10
					+ memory.getInteger(5).intValue()*1;
		int Bchem = memory.getInteger(6).intValue()*100
					+ memory.getInteger(7).intValue()*10
					+ memory.getInteger(8).intValue()*1;
		int Bcoef = memory.getInteger(9).intValue()*100
					+ memory.getInteger(10).intValue()*10
					+ memory.getInteger(11).intValue()*1;
		int Cchem = memory.getInteger(12).intValue()*100
					+ memory.getInteger(13).intValue()*10
					+ memory.getInteger(14).intValue()*1;
		int Ccoef = memory.getInteger(15).intValue()*100
					+ memory.getInteger(16).intValue()*10
					+ memory.getInteger(17).intValue()*1;
		int Dchem = memory.getInteger(18).intValue()*100
					+ memory.getInteger(19).intValue()*10
					+ memory.getInteger(20).intValue()*1;
		int Dcoef = memory.getInteger(21).intValue()*100
					+ memory.getInteger(22).intValue()*10
					+ memory.getInteger(23).intValue()*1;
		int KMVM = memory.getInteger(24).intValue()*100
					+ memory.getInteger(25).intValue()*10
					+ memory.getInteger(26).intValue()*1;
	...
	private static BrainGene getBG(
			boolean mutate,boolean duplicate,boolean delete,boolean activ,
			int age_min,int age_max,int sex,int mut_rate,IntegerListe memory) {
		int height = memory.getInteger(0).intValue()*10
					+ memory.getInteger(1).intValue()*1;
		int width = memory.getInteger(2).intValue()*10
					+ memory.getInteger(3).intValue()*1;
		int depth = memory.getInteger(4).intValue()*10
					+ memory.getInteger(5).intValue()*1;
		int more = memory.getInteger(6).intValue()*10
					+ memory.getInteger(7).intValue()*1;
	...
	private static BrainLobeGene getBLG(
			boolean mutate,boolean duplicate,boolean delete,boolean activ,
			int age_min,int age_max,int sex,int mut_rate,IntegerListe memory) {
		int rest = memory.getInteger(0).intValue()*100
					+ memory.getInteger(1).intValue()*10
					+ memory.getInteger(2).intValue()*1;
		int thre = memory.getInteger(3).intValue()*100
					+ memory.getInteger(4).intValue()*10
					+ memory.getInteger(5).intValue()*1;
		int desc = memory.getInteger(6).intValue()*100
					+ memory.getInteger(7).intValue()*10
					+ memory.getInteger(8).intValue()*1;
		int dendriticmin = memory.getInteger(9).intValue()*100
					+ memory.getInteger(10).intValue()*10
					+ memory.getInteger(11).intValue()*1;
		int dendriticmax = memory.getInteger(12).intValue()*100
					+ memory.getInteger(13).intValue()*10
					+ memory.getInteger(14).intValue()*1;
		int prox = memory.getInteger(15).intValue()*100
					+ memory.getInteger(16).intValue()*10
					+ memory.getInteger(17).intValue()*1;
		boolean repr = (memory.getInteger(18).intValue()%2 == 0);
		int repy = memory.getInteger(19).intValue()*100
					+ memory.getInteger(20).intValue()*10
					+ memory.getInteger(21).intValue()*1;
		boolean wta = (memory.getInteger(22).intValue()%2 == 0);
		int height = memory.getInteger(23).intValue()*10
					+ memory.getInteger(24).intValue()*1;
		int width = memory.getInteger(25).intValue()*10
					+ memory.getInteger(26).intValue()*1;
		int posx = memory.getInteger(27).intValue()*10
					+ memory.getInteger(28).intValue()*1;
		int posy = memory.getInteger(29).intValue()*10
					+ memory.getInteger(30).intValue()*1;
		boolean replace = (memory.getInteger(31).intValue()%2 == 0);
	...
	private static EmitterReceptor getER(
		...
		int var = memory.getInteger(0).intValue()*100
					+ memory.getInteger(1).intValue()*10
					+ memory.getInteger(2).intValue()*1;
		int thr = memory.getInteger(3).intValue()*100
					+ memory.getInteger(4).intValue()*10
					+ memory.getInteger(5).intValue()*1;
		int put = memory.getInteger(6).intValue()*100
					+ memory.getInteger(7).intValue()*10
					+ memory.getInteger(8).intValue()*1;
		int posx = memory.getInteger(9).intValue()*10
					+ memory.getInteger(10).intValue()*1;
		int posy = memory.getInteger(11).intValue()*10
					+ memory.getInteger(12).intValue()*1;
		boolean receptor = (memory.getInteger(13).intValue()%2 == 0);
		boolean internal = (memory.getInteger(14).intValue()%2 == 0);
	...
	private static StimulusDecision getSD(
			boolean mutate,boolean duplicate,boolean delete,boolean activ,
			int age_min,int age_max,int sex,int mut_rate,IntegerListe memory) {
		boolean perc = (memory.getInteger(0).intValue()%2 == 0);
		boolean obje = (memory.getInteger(1).intValue()%2 == 0);
		int indi = memory.getInteger(2).intValue()*100
				+ memory.getInteger(3).intValue()*10
				+ memory.getInteger(4).intValue()*1;
		int thre = memory.getInteger(5).intValue()*100
				+ memory.getInteger(6).intValue()*10
				+ memory.getInteger(7).intValue()*1;
		int attr = memory.getInteger(8).intValue()*100
				+ memory.getInteger(9).intValue()*10
				+ memory.getInteger(10).intValue()*1;
		int vari = memory.getInteger(11).intValue()*100
				+ memory.getInteger(12).intValue()*10
				+ memory.getInteger(13).intValue()*1;
		int valu = memory.getInteger(14).intValue()*100
				+ memory.getInteger(15).intValue()*10
				+ memory.getInteger(16).intValue()*1;
		int scri = memory.getInteger(17).intValue()*100
				+ memory.getInteger(18).intValue()*10
				+ memory.getInteger(19).intValue()*1;
	...
	private static Instinct getIN(
			boolean mutate,boolean duplicate,boolean delete,boolean activ,
			int age_min,int age_max,int sex,int mut_rate,IntegerListe memory) {
		int in_posx = memory.getInteger(0).intValue()*10
					+ memory.getInteger(1).intValue()*1;
		int in_posy = memory.getInteger(2).intValue()*10
					+ memory.getInteger(3).intValue()*1;
		int out_posx = memory.getInteger(4).intValue()*10
					 + memory.getInteger(5).intValue()*1;
		int out_posy = memory.getInteger(6).intValue()*10
					 + memory.getInteger(7).intValue()*1;
		int weight = memory.getInteger(8).intValue()*100
				+ memory.getInteger(9).intValue()*10
				+ memory.getInteger(10).intValue()*1;
		int var = memory.getInteger(11).intValue()*100
				+ memory.getInteger(12).intValue()*10
				+ memory.getInteger(13).intValue()*1;
		int thr = memory.getInteger(14).intValue()*100
				+ memory.getInteger(15).intValue()*10
				+ memory.getInteger(16).intValue()*1;
		boolean check = (memory.getInteger(17).intValue()%2 == 0);
		return new Instinct(
				mutate,duplicate,delete,activ,age_min,age_max,sex,mut_rate,
