...
public class OrganismSaveJPanel extends GeneKitsGBJPanel {
	...
	private void saveOrganism() {
		...
		if (this.geneScroll != null) {
			StringListe genesNamesList	= this.geneScroll.getStringListe();
			Gene tmp = null;
			for (int i = 0 ; i < genesNamesList.length() ; i++) { 
				tmp = this.geneStock.getGene(genesNamesList.getString(i));
				if (tmp == null) {
					for (int j = 0 ; (j < this.pathStock.length()) && (tmp == null) ; j++) {
						Pathway path = this.pathStock.getPathway(j);
						tmp = path.getGeneMoreListe().getGene(genesNamesList.getString(i));
					}
				} 
				// if (tmp != null) { currentChromosome.addGene(tmp); }
				if ( (genesNamesList.getString(i).matches("^-*$")) 
						|| (tmp == null) ) {
...
//						(GeneGattaca.getInstance(genesNamesList.getString(i))); }
		...
		if (pathAndFileName != null) {
			if (this.orgSelectPanel != null) {
				...
				this.toSave.getOrganism()
					.setOrganismType(this.orgSelectPanel.getTypeOrganism());
			...
			if (this.orgNamesPanel != null) {
				...
				for (int i = 0 ; i < this.orgNamesPanel.getOthersNames().length() ; i++) 
					{ this.toSave.addOtherName(this.orgNamesPanel
												.getOthersNames().getString(i)); }
			}
