...
public class GeneticTreeNodeTests01 {
	...
	public static void main (String argv[]) {	
		...
		try {
			...
			Terminal.ecrireStringln("root : "+testRootGattaca.getNodes().size()+"\t"+testRootGattaca.getCodeValues());
			Terminal.sautDeLigne();
			Terminal.sautDeLigne();
			
			// List<GeneticTreeNode> level1bis = testRoot.getNodes();
			// for (int i = 0 ; i < level1bis.length() ; i++) { level1bis.get(i).computeNode(); }
			
			List<GeneticTreeNode> level1 = testRootGattaca.getNodes();
			for (int i = 0 ; i < level1.size() ; i++) {
				GeneticTreeNode tmpLeave1 = level1.get(i);
				...
				List<GeneticTreeNode> level2 = tmpLeave1.getNodes();
				for (int j = 0 ; j < level2.size() ; j++) {
					GeneticTreeNode tmpLeave2 = level2.get(j);
					...
					List<GeneticTreeNode> level3 = tmpLeave2.getNodes();
					for (int k = 0 ; k < level3.size() ; k++) {
						GeneticTreeNode tmpLeave3 = level3.get(k);
			...
			List<GeneticTreeNode> testLeaves = testRootGattaca.getLeaves();
			for (int i = 0 ; i < testLeaves.size() ; i++) {
				GeneticTreeNode tmpLeave = testLeaves.get(i);
		...
		try {
			...
			List<GeneticTreeNode> testFeuilles = testRootPhaseTwo.getLeaves();
			Terminal.ecrireStringln(testFeuilles.size()+"");
			for (int i = 0 ; i < testFeuilles.size() ; i++) {
				GeneticTreeNode tmpFeuille = testFeuilles.get(i);
