...
public class World implements VariableContent {
	...
	private Chemicals half_lives;
	
	public static final int MAX_HEIGHT = 20;
	public static final int MAX_WIDTH = 20;
	
	/** Default construcotr of global environment. */
	public World() {
		this.half_lives = new Chemicals();
	...
	public void execution() {
		...
		for (int i = 0 ; i < this.map.length ; i++) {
			for (int j = 0 ; j < this.map[i].length ; j++) { 
				...
				/** Movement of agents */
				for (int k = 0 ; k < this.map[i][j].getAgentListLength() ; k++)
					{ this.map[i][j].getAgent(k).deplace(); }
			}
		}
	}
	...
	public Chemicals getVariables() { return this.half_lives; }
